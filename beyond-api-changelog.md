...
* `PUT` /carts/{cardId}/customer Assign customer to cart  
    Return Type
        ...
        Add taxable //Indicates if a tax applies for the cart. Can be `true` or `false`.
        Add balanceDue
        Add grandTotal
        Add taxTotal
        Add weight //The total weight of products in the cart. A product's weight defaults to 0 if not set.
        Add taxes //The tax amount per tax class.
        Add checkoutState
        Add couponLineItem //The object which contains discount details.
        Add minimumOrderValue
        Add lineItems //The list of line items.
        Add shippingLineItem
        Add subtotal
        Add customerEmail //The customer's email address that is assigned to the cart.
        Add shippingAddress
        Add mustAcceptTermsAndConditions //Indicates if the customer has to explicitly consent to the terms and conditions before buying something.
        Add netTotal
        Add billingAddress
...
* `PUT` /carts/{cartId}/billing-address Set cart billing address  
    Return Type
        ...
        Add subtotal
        Add customerEmail //The customer's email address that is assigned to the cart.
        Add shippingAddress //The shipping address that applies to the cart. Defaults to billing address if no shipping address is set. Refer to <<resources-cart-set-shipping-address,Set cart shipping address>> for item documentation.
        Add mustAcceptTermsAndConditions //Indicates if the customer has to explicitly consent to the terms and conditions before buying something.
        Add billingAddress
        Add netTotal
        Add _id //The immutable, unique identifier of the requested resource.
...
* `POST` /carts/{cartId}/coupon Redeem coupon  
    Return Type
        ...
        Add mustAcceptTermsAndConditions //Indicates if the customer has to explicitly consent to the terms and conditions before buying something.
        Add netTotal
        Add billingAddress //The billing address of the cart. Refer to <<resources-cart-set-billing-address,Set cart billing address>> for item documentation.
        Add _id //The immutable, unique identifier of the requested resource.
        Add paymentLineItem
...
* `GET` /carts/{cartId}/payment-methods/current Show current cart payment method details  
    Return Type
        ...
        Add activeLogoSet //The name of the currently selected logo set. If no value is set, the official logo set is used.
        Add defaultPaymentNote //The payment note for the payment method to be printed on invoice documents.
...
* `GET` /carts/{cartId}/shipping-methods/current Show current cart shipping method details  
    Return Type
        ...
        Add taxClass //The tax class of the line item. Can be one of `REGULAR`, `REDUCED`, or `EXEMPT`.
        Add freeShippingValue
        Add name //The name of the shipping method.
        Add description //The description of the shipping method.
        Add weightBasedPrice //The price depending on the package weight (optional).
        Add position //The sorting position of the shipping method in the merchant's cockpit in relation to other shipping methods.
        Add _id //The immutable, unique identifier of the requested resource.
        Add fixedPrice
...
* `PUT` /coupon-campaigns/{couponCampaignId}/status Update coupon campaign status  
    Return Type
        ...
        Add maxNumberOfRedemptions //The maximum number of coupon code redemptions for a campaign (optional). It is validated against the aggregated redemptions counter of all coupon codes that are part of the coupon campaign.
        Add discounts
        Add _links //See https://developer.epages.com/beyond-docs/#hypermedia[Hypermedia]
        Add start //The start date and time of the coupon campaign. Coupons of the coupon campaign cannot be redeemed before. Expressed according to ISO 8601, e.g. `2020-11-01T00:00:00`. If omitted or set to `null` in the create/update request, it defaults to the current date and time.
        Add name //The title of the coupon campaign.
        Add active //(Deprecated) Indicates whether a coupon campaign is active or not. Can be `true` or `false`. The value of this property impacts the status of the coupon campaign (see <<resources-coupon-campaign-update>>).
        Add totalNumberOfRedemptions //The sum of the redemption counters of all coupon codes in the given campaign.
        Add end //The end date and time of the coupon campaign (optional). Coupons of the coupon campaign cannot be redeemed afterwards. Expressed according to ISO 8601, e.g. `2020-12-31T12:00:00`.
        Add _id //The immutable, unique identifier of the requested resource.
        Add status //The information whether the coupon campaign is `ACTIVE` and customers and redeem coupons. If it is not `ACTIVE`, the status explains why not. Can be {{values}}. After the creation of a campaign, the status is `DRAFT`. Also see <<resources-coupon-campaign-status-update>>.
* `GET` /orders/search/find-by-cart-id Show order by cart ID  
    Return Type
        ...
        Add customerComment //The comment from the customer regarding their order.
        Add _links //See https://developer.epages.com/beyond-docs/#hypermedia[Hypermedia]
        Add taxModel //The tax model that applies for the order.
        Add orderNote //The merchant comment or note on the order.
        Add taxTotal
        Add taxes
        Add productLineItems
        Add salesChannel //The sales channel for the order, such as an online shop, point of sale, or a retail store.
        Add shippingStatus //The current shipping status of the order. Can be one of `UNSHIPPED`, `PARTIALLY_SHIPPED`, `PARTIALLY_RETURNED`, `SHIPPED`, `PENDING`, `DELIVERED`, `CANCELED`, or `RETURNED`.
        Add createdAt //The date and time the order was technically created in the system. According to ISO 8601. Can deviate from `entryDate`, e.g. because the order was originally created earlier or in another system.
        Add termsAndConditionsExplicitlyAccepted //Indicates if the customer explicitly accepted the shop's terms and conditions before ordering. Can be `true` or `false`.
        Add initialBalanceDue
        Add initialGrandTotal
        Add externalPaymentUri //The link to the payment in a payment provider's administration if provided by the payment app.
        Add customerEmail //The email address of the registered customer.
        Add customerId //The unique identifier of the customer who placed the order. Only provided for registered customers.
        Add externalPaymentId //The external payment reference if provided by the payment app.
        Add totalRefunded
        Add totalReceived
        Add testOrder //Indicates if the order is a test order. Can be `true` or `false`.
        Add currency //The currency in which the order was entered.
        Add marketingChannel //The marketing channel for the order, such as a newsletter campaign, Google Shopping, or a Buy Button that’s implemented on a blog.
        Add paymentStatus //The current status of the payment associated with the order. Can be one of `PAID`, `PARTIALLY_PAID`, `PENDING`, `REFUND_PENDING`, or `VOIDED`.
        Add paymentLineItem
        Add updatedAt //The date and time the order was last updated. According to ISO 8601.
        Add marketingSubchannel //The marketing subchannel for the order, such as a specific newsletter of a newsletter campaign, e.g. a Summer Sale that’s promoted on a blog, or a Winter Special that’s promoted on Facebook.
        Add taxable //Indicates if a tax applies for the order. Can be `true` or `false`.
        Add balanceDue
        Add grandTotal
        Add entryDate //The date and time of the actual creation of the order from a business perspective. Can deviate from `createdAt`.
        Add cartId //The ID of the cart the order has been created from (if existing).
        Add customerNumber //The customer number of the customer who placed the order. Only provided for registered customers.
        Add openAmount
        Add couponLineItem //The coupon associated with the order.
        Add canceled //Indicates if the order was canceled. Can be `true` or `false`.
        Add paymentNote //A prompt to pay the order which will be rendered in the invoice order document, e.g. a payment reference and the merchant's bank account details.
        Add backgroundJobs //(Deprecated) Pending processes initiated via the cockpit or the API that run in the background, e.g. rendering an invoice document. Instead, please refer to the links section of the respective order resource to get an indication whether the rendering of an order document is completed.
        Add initialNetTotal
        Add shippingLineItem
        Add subtotal
        Add shippingAddress //The shipping address of the cart. Defaults to billing address if no shipping address is set. Refer to <<resources-cart-set-shipping-address,Set cart shipping address>> for item documentation.
        Add billingAddress //The billing address of the cart. Refer to <<resources-cart-set-billing-address,Set cart billing address>> for item documentation.
        Add netTotal
        Add _id //The unique identifier of the order.
        Add initialSubtotal
        Add initialTaxTotal
...
* `GET` /orders/{orderId}/processes/payments/active Show active payment process details  
    Return Type
        ...
        Add captureWorkflow //Indicates how and when to capture an authorized payment. Can be one of {{values}} for online payments or `null` if it is an offline payment.
        Add amount
        Add _links //See https://developer.epages.com/beyond-docs/#hypermedia[Hypermedia]
        Add paymentId //The ID of the payment created by the payment gateway.
        Add _id //The ID of the refund process.
        Add completed //Indicates if the payment has been completed. Can be `true` or `false`.
        Add status //The status of the payment process. Can be one of {{values}}.
* `GET` /orders/{orderId}/processes/refunds/active Show active refund process details  
    Return Type
        ...
        Add _links //See https://developer.epages.com/beyond-docs/#hypermedia[Hypermedia]
        Add _id //The ID of the refund process.
        Add completed //Indicates if the refund has been completed. Can be `true` or `false`.
        Add status //The status of the refund process. Can be one of {{values}}.
...
* `POST` /orders/{orderId}/processes/shippings/{shipping-process-id}/mark-delivered Mark shipment as delivered  
    Return Type
        ...
        Add trackingLink //The tracking link for the shipping process.
        Add _links //See https://developer.epages.com/beyond-docs/#hypermedia[Hypermedia]
        Add trackingCode //The tracking code for the shipping process.
        Add _id //The ID of the shipping process.
        Add status //The status of the shipping process. Can be one of {{values}}.
...
* `PUT` /shipping-zones/{shippingZoneId} Update shipping zone  
    Return Type
        ...
        Add name //The name of the shipping zone.
        Add position //The sorting position of the shipping zone in the merchant's cockpit in relation to other shipping zones.
        Add _id //The immutable, unique identifier of the requested resource.
        Add serviceableCountries //The list of countries this shipping zone applies to.
        Add shippingMethods //The shipping methods assigned to this shipping zone.
...
* `POST` /webhook-subscriptions/{subscriptionId}/activate Activate webhook subscription  
    Return Type
        ...
        Add callbackUri //The callback URI of the webhook.
        Add active //Indicates if this subscription is active. Can be `true` or `false`.
        Add _id //The immutable, unique identifier of the requested resource.
        Add eventTypes //The event type names the webhook listens to.
