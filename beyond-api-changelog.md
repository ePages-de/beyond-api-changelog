...
### What's New
---
* `POST` /callback Order webhook request against callback URI
* `POST` /carts/{cartId}/customer Create customer

### What's Deprecated
---
* `POST` /carts/{cart_id}/customer Create customer

### What's Changed
---
* `POST` /oauth/token Create a JSON Web Token with client credentials  
    Return Type

        Delete refresh_token //The token that can be used to obtain a renewed access token.
...
* `POST` /orders/{orderId}/processes/shippings Create shipping process  
    Parameter
        ...
        Add .createPackingSlip //Indicates if a packing slip pdf is created as a part of the shipping process.
* `POST` /orders/{orderId}/processes/shippings/{shipping-process-id}/mark-shipped Mark shipment as shipped  
    Parameter

        Delete .sendMail //Indicates if an email is to be sent out to the customer to indicate the shipping process transition.
* `GET` /payment-methods/{paymentMethodId} Show payment method details  
    Return Type
        ...
        Add description //Description of the payment method that is used during checkout.
...
* `GET` /product-view/products/{productId} Show product details  
    Return Type
        ...
        Add listPriceText //The text that comes along with the list price.
        Add sku //The stock keeping unit (SKU) corresponding to the product.
        Delete variations //The variations of the variation product.
        Delete maxSalesPrice //The maximum price of the product.
        Delete minSalesPrice //The minimum price of the product.
        Delete variationAttributes //The variation attributes with its values.
* `GET` /product-view/categories/{categoryId}/products List products of category  
    Return Type

        Delete _links //See <<hypermedia,Hypermedia>>
        Delete _embedded
        Delete page //See <<pagination,Pagination>>
...
* `GET` /shipping-zones/{shippingZoneId}/shipping-methods/{shippingMethodId} Show details of shipping method in shipping zone  
    Return Type
        ...
        Delete weightBasedPrice //The price depending on the package weight (optional).
