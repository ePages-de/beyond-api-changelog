...
* `POST` /oauth/token Create a JSON Web Token with a refresh token  
    Return Type
        ...
        Add refresh_token //The token that can be used to obtain a renewed access token.
        Add userId //The unique identifier of the user the token was issued for.
        Add username //The username that identifies the user the token was issued for.
* `GET` /payment-methods/{paymentMethodId} Show charged payment method details  
    Return Type

        Delete taxClass //The tax class. Can be one of `REGULAR`, `REDUCED`, or `EXEMPT`.
        Delete onlinePayment //Indicates if the payment method is an online payment, i.e. the payment is processed by an external electronic payment system. Can be `true` or `false`.
        ...
        Delete position //The sorting position of the payment method in the merchant's cockpit in relation to other payment methods.
...
* `GET` /shipping-zones/{shippingZoneId}/shipping-methods Sort shipping methods in shipping zone  
    Return Type
